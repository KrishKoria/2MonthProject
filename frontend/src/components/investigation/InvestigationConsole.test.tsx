...
test("renders recorded human review details when a decision already exists", async () => {
  ...
  expect(/<button[^>]*disabled[^>]*>[\s\S]*?Approve[\s\S]*?<\/button>/.test(html)).toBe(true);
  expect(/<button[^>]*disabled[^>]*>[\s\S]*?Stop payment[\s\S]*?<\/button>/.test(html)).toBe(true);
  expect(/<button[^>]*disabled[^>]*>[\s\S]*?Escalate[\s\S]*?<\/button>/.test(html)).toBe(true);
});
