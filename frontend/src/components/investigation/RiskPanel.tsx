    ...
    () =>
      riskScore
        ? Object.entries(riskScore.shap_values)
            ...
            .map(([feature, value]) => ({
              feature,
              label: getFriendlyFeatureLabel(feature),
              value,
            }))
        : [],
    [riskScore],
  );
  const shapMax = Math.max(...topShap.map((item) => Math.abs(item.value)), 0.0001);
    ...
    <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-display text-2xl italic capitalize">
              {riskBand ? RISK_BAND_COPY[riskBand].label : "Not scored yet"}
            </span>
          </div>
          {riskBand ? (
            <p className="max-w-[26ch] text-xs leading-relaxed text-muted-foreground">
              {RISK_BAND_COPY[riskBand].description}
            </p>
          ) : null}
      ...
      <div>
        <div className="mb-3 flex items-center gap-1.5 text-[11px] uppercase tracking-[0.14em] text-muted-foreground">
          <span>What moved the score</span>
          <HelpTooltip label="What moved the score">{TERM_COPY.shap}</HelpTooltip>
        </div>
        {topShap.length === 0 ? (
          <p className="text-xs text-muted-foreground">
            This claim does not have a score breakdown yet.
          </p>
        ) : (
          <ul className="flex flex-col gap-3">
            {topShap.map((item, i) => {
              const pct = (Math.abs(item.value) / shapMax) * 100;
                ...
                <li key={item.feature} className="flex flex-col gap-1">
                  <div className="flex items-baseline justify-between text-xs">
                    <div className="flex items-center gap-1.5">
                      ...
                      <HelpTooltip label="Technical model field">
                        This label is translated into plain language from an internal
                        model field name.
                      </HelpTooltip>
                    </div>
